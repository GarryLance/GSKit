...
@interface GSCameraManager () <AVCaptureVideoDataOutputSampleBufferDelegate, CAAnimationDelegate ,AVCaptureMetadataOutputObjectsDelegate>
{
    GSView * _viewPreview;
}
...
dispatch_queue_t captureSessionQueue() {
    ...
    dispatch_once(&queueCreationGuard, ^{
        queue = dispatch_queue_create("com.gskit.captureSessionQueue", 0);
    });
    return queue;
}
...
- (void)start {
    ...
    dispatch_async(captureSessionQueue(), ^{
        [_self.captureSession startRunning];
    });
}
...
- (BOOL)changeTourchMode:(AVCaptureTorchMode)tourchMode
{
    ...
    if (![device isTorchModeSupported:tourchMode])
    {
        GSDLog(@"不支持该手电筒模式");
        return NO;
    }
